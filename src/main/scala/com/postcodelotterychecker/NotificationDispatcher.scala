...
class NotificationDispatcher(emailClient: EmailClient)(implicit executionContext: ExecutionContext) {
  ...
  def dispatchNotifications(allUsers: List[User], postcodeUserResults: Map[User, Option[Boolean]], winningPostcode: Postcode, dinnerUserResults: Map[User, Option[Boolean]], winningDinnerUsers: List[DinnerUserName], stackPotUserResults: Map[User, Option[Boolean]], winningStackpotPostcodes: List[Postcode], emojiUserResults: Map[User, Option[Boolean]], winningEmojiSequence: Set[Emoji]): Future[Unit] = {
    Future {
       allUsers.foreach(user => {
        ...
        val combinedResult: Option[Boolean] = postcodeResult.toList ::: dinnerResult.toList ::: stackpotResult.toList ::: emojiResult.toList match {
          case Nil => None
          case list =>  Some(list.exists(identity))
        }

         def generateMessageBody(postcodeResult: Option[Boolean], dinnerResult: Option[Boolean], stackpotResult: Option[Boolean], emojiResult: Option[Boolean]): String = {
           s"""
              |Today's Results
              |
              |${postcodeResult.fold("") { result =>
                ...
                s"You are watching the following postcodes: ${user.postCodesWatching.map(_.map(_.value).mkString(", ")).getOrElse("N/A")} \n" }}
              ...
              |${dinnerResult.fold("") { result => s"Win A Dinner: ${if (result) "WON" else "Not won"} \n" +
                ...
                s"You are watching the following users: ${user.dinnerUsersWatching.map(_.map(_.value).mkString(", ")).getOrElse("N/A")}"}}
        ...
        combinedResult.foreach(result => {
          ...
          val email = Email(
            ...
            body = generateMessageBody(postcodeResult, dinnerResult, stackpotResult, emojiResult)
          )
